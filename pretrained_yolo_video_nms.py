...
file_video_stream = cap = cv2.VideoCapture(0)
file_video_stream.set(cv2.CAP_PROP_FPS,30)
...
while (file_video_stream.isOpened):
    ...
    img_blob = cv2.dnn.blobFromImage(img_to_detect, 0.003922, (416, 416), swapRB=True, crop=True)
    ...
    class_labels = ["cans","glass","hdpe","pet"]
    ...
    yolo_model = cv2.dnn.readNetFromDarknet('dataset/yolov4-obj.cfg','dataset/yolov4-obj_best.weights')
    ...
    ############## NMS Change 3 END ########### 
        ...
        cv2.putText(img_to_detect, f'FPS: {int(fps)}', predicted_class_label, (start_x_pt, start_y_pt-5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, box_color, 1)
